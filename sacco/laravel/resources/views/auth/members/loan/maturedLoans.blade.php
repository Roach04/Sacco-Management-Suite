			          	...
			          	<div class="col-lg-12">
	              			<div class="card">
	                			...
	                			<div class="card-body">
								          		...
								          		@if($loan->loanEntity == 'individual' && Carbon\Carbon::now() >= $loan->created_at->addDays($loan->gracePeriod))
								          			<tbody class="tbuddy">
								                      	<tr>
									                        ...
									                        <td> 
								                        		<a class="btn btn-warning" data-id="{{ $loan->id }}" data-account="{{ $loan->member->accountNumber }}" data-lname="{{ $loan->member->lastname }}" data-fname="{{ $loan->member->firstname }}" id="modal-loan-install" href="#installmentsmodal" data-toggle="modal"> 
									                        		Insertions
									                        	</a>
									                        </td>
									                        ...
									                        @if(count($loan->installments))
										                        <td> 
										                        	<a href="{{ route('installmentUpdates', [$loan->member->id]) }}" class="btn btn-primary"> 
										                        		Edit 
										                        	</a> 
										                        </td>
									                        @endif
					                ...
					                @else
						        		<div class="container">
			                                <p style="color:#ffc36d; font-size:26px; text-align:center"> There are No Members with Matured Loans at This Time. </p>
			                            </div>
							        @endif
							    </div>
						    </div> 
