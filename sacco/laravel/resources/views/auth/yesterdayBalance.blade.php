        ...
        <div class="content-inner">
         	...
         	<header class="page-header">
	            <div class="container-fluid">
	            	<h2 class="no-margin-bottom">{{ $yesterdayEnd->formatlocalized('%a %d %b %y') }}
          	...
          	<ul class="breadcrumb">
            	<div class="container-fluid">
              		...
              		<li class="breadcrumb-item active">
	              		{{ $yesterdayEnd->formatlocalized('%B %Y') }} 
              		</li>
              		@if(count($today))
              			<div class="pull-right">
	          		...
	          		<div class="row">
	            		...
	            		<div class="col-lg-12">
	              			<div class="card">
	                			...
	                			<div class="card-body">
	                				@if(count($closingBalanceYesterday) > 0)
							            <div class="row">
		                					<label class="col-sm-3 form-control-label">{{ $yesterdayEnd->formatlocalized('%A %d') }} Balances</label>
								          	<div style="margin-top:40px" class="col-lg-12 col-md-12 col-sm-12 col-xs-12"> 
								                <table id="table-data" style="text-align:center; width:100%" class="table table-bordered table-hover">
								                	...
								                	@else
								                		<tbody>
								                			<tr class="table-warning">
								                				<td colspan="8">No Cashbook Transaction as of {{ $yesterdayEnd->formatlocalized('%a %d %b %y') }}</td>
								                			</tr>
								                		</tbody>
								                	@endif
								                	<!-- Journals -->
								                	...
								                	@else
								                		<tbody>
								                			<tr class="table-warning">
								                				<td colspan="8">No Journal Transaction as of {{ $yesterdayEnd->formatlocalized('%a %d %b %y') }}</td>
								                			</tr>
								                		</tbody>
								                	@endif
								                	<!-- Member Deposits -->
								                	...
								                	@else
								                		<tbody>
								                			<tr class="table-warning">
								                				<td colspan="8">No Member Deposits as of {{ $yesterdayEnd->formatlocalized('%a %d %b %y') }}</td>
								                			</tr>
								                		</tbody>
								                	@endif
								                	<!-- Loan Disbursements -->
								                	...
								                	@else
								                		<tbody>
								                			<tr class="table-info">
								                				<td colspan="8">No Loan Disbursements as of {{ $yesterdayEnd->formatlocalized('%a %d %b %y') }}</td>
								                			</tr>
								                		</tbody>
								                	@endif
								                	<!-- Loan Payments -->
									                ...
									                @else
								                		<tbody>
								                			<tr class="table-danger">
								                				<td colspan="8">No Loan Reimbursements as of {{ $yesterdayEnd->formatlocalized('%a %d %b %y') }}</td>
								                			</tr>
								                		</tbody>
								                	@endif
								                </table>

								                <a href="{{ route('pdfYesterdayBalance') }}" class="pull-right col-lg-4 col-md-4 btn btn-info">
								                	Print Yesterday's Balance
								                </a>
								            </div>
							            </div>
							        @else
				                		<div class="container">
						            		<p style="color:red; text-align:center; font-size:24px; padding-left:30px"> There were NO Sacco Transactions as of {{ $yesterdayEnd->formatlocalized('%a %d %b %y') }}. </p>
						            	</div>
						            @endif
	                			</div>
	                		</div>
